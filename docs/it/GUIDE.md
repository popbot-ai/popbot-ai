...
- **Esegui nel cloud.** Quando crei una chat, attiva il pulsante *Cloud* accanto al selettore dell'agente e la chat guiderà una [sessione cloud di Claude Code](https://claude.ai/code) invece di un agente locale: funziona con qualsiasi scelta di spazio di lavoro: senza repository la sessione gira semplicemente in remoto; dalla radice del repository il cloud clona il remoto GitHub sul branch su cui si trova la radice (fai prima il push); con uno slot o un worktree la chat riceve il suo branch come al solito, PopBot lo invia a `origin` e il cloud clona quel branch mentre lo slot resta locale per riportare indietro il lavoro col teletrasporto. Il primo messaggio avvia la sessione dal terminale della chat (`claude --cloud`) e, finché quella CLI resta collegata, la conversazione viene rispecchiata nella colonna della chat — risposte, chiamate agli strumenti e tutto il resto — e i messaggi che invii vengono digitati direttamente lì. Quando la CLI è terminata, i messaggi vengono ancora accodati nella sessione (`claude -p --cloud`), ma le risposte si vedono solo su claude.ai. La sessione continua a girare dopo la chiusura di PopBot. Il cloud raggiunge GitHub tramite il tuo account claude.ai, non la tua macchina: installa la Claude GitHub App da claude.ai/code oppure esegui `/web-setup` una volta in una sessione `claude` nel terminale. Il chip nell'intestazione rimanda alla sessione su claude.ai, e *Teletrasporta nel terminale* nelle impostazioni della chat riporta branch e conversazione sulla tua macchina. Per ora solo Claude; Codex Cloud non è collegato.
