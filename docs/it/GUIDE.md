...
- L'archivio è ricercabile per nome, ticket, branch e contenuto, e **Cerca nelle chat** (⌘⇧F / Ctrl+Maiusc+F, oppure *Visualizza ▸ Cerca nelle chat…*) cerca in tutte le trascrizioni, aperte e archiviate: le righe trovate sono raggruppate per chat, le migliori prima, e *Vai* apre la chat — riaprendola dall'archivio se serve — e scorre fino a quella riga. I tag di filtro restringono la ricerca — `ticket:ENG-123`, `cr:67`, `last:week`, `from:user`, `tool:Bash`, `agent:codex`, `in:archive`, `chat:nome` — digitati nella casella o scelti con i pulsanti sotto; i soli tag elencano le voci corrispondenti più recenti. Digitando `ticket:`, `cr:` o `chat:` compare sotto la casella l'elenco dei ticket, delle PR e delle chat che PopBot conosce; Tab ne accetta uno.
