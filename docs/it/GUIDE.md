...
Cinque controlli di sessione contano per il lavoro di lunga durata:
...
- **Dirama la chat.** Dal menu ☰ della chat (o dal suo pannello impostazioni), *Dirama chat* avvia una seconda chat esattamente da questo punto, così le due possono divergere: la conversazione finora viene copiata, la sessione dell'agente viene diramata (Claude e Codex lo supportano nativamente, quindi la diramazione ricorda il lavoro invece di rileggerlo) e una chat con spazio di lavoro riceve un proprio slot nuovo su un branch `<branch>-fork` con le modifiche non committate dell'originale. L'originale prosegue intatta e all'agente della diramazione viene indicato dove si trova la cartella d'origine, così ricrea i worktree specifici della chat invece di riutilizzarli.
- **Esegui nel cloud.** Quando crei una chat, attiva il chip *Cloud* accanto al selettore dell'agente e la chat gira su [Anthropic Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview) invece che in una CLI locale: una sessione in una sandbox cloud di Anthropic, fatturata a token sulla chiave API impostata in Preferenze ▸ Agenti ▸ Chat cloud, che continua a lavorare quando PopBot viene chiuso. Funziona con qualsiasi scelta di workspace: senza repo la sandbox è vuota; dalla radice del repo la sandbox clona il remoto GitHub sul branch su cui si trova la radice (con push prima, se `origin` non lo ha ancora); con uno slot o un worktree la chat riceve il suo branch come al solito, PopBot lo pusha su `origin` e la sandbox lo clona. La sandbox clona con il tuo token GitHub (quello delle Preferenze, o quello con cui `gh` ha effettuato l'accesso), e l'agente riceve quel token dentro la sandbox per poter fare push; gli viene detto di tenerlo fuori dalle risposte, e PopBot non lo salva mai nella chat. Il tuo primo messaggio crea la sessione; risposte, chiamate agli strumenti e risultati arrivano nella chat come con qualsiasi altro agente, e quanto accaduto mentre PopBot era chiuso viene riprodotto alla successiva apertura della chat. Gli strumenti girano nella sandbox senza richieste di permesso (è isolata; l'unica cosa che ne esce è il push sul branch della chat). L'agente fa commit e push su quel branch; *Porta il lavoro del cloud nel checkout* nel menu della chat (o nelle impostazioni della chat) porta il checkout locale a quel punto in fast-forward. Cambiare modello o effort termina la sessione; il messaggio successivo ne avvia una nuova, preparata con la conversazione. Chiudere la chat, eliminarla o uscire da PopBot non termina mai la sessione sul server: è così che continua a lavorare senza di te. *Spegni la chat cloud* nel menu della chat (☰, o clic destro sul chip Cloud) è ciò che la termina: se sta ancora lavorando viene interrotta, poi archiviata; quanto non pushato va perso, e il messaggio successivo avvia una nuova sessione preparata con la conversazione. Solo Claude; Codex Cloud non è collegato.
- **Esegui su un'altra macchina.** *Esegui su*, nella finestra di nuova chat, elenca gli host di Preferenze ▸ Host: altre macchine su cui gira `popbot-host`, un piccolo demone che esegue le CLI `claude` e `codex` nei propri checkout. Scegline uno e la finestra mostra i repository e i branch di quell'host al posto di quelli locali: la chat gira nella radice del repository, in un worktree che l'host crea su un nuovo branch, oppure in una cartella temporanea lì senza repository. Sulla tua macchina non viene creato nulla; trascrizione, ricerca, impostazioni e regole dei permessi restano nel database di PopBot, e le risposte dell'agente, le chiamate agli strumenti e le richieste di permesso arrivano nella chat come sempre (gli allegati vengono inviati). Il chip della chat nomina l'host e apre le sue impostazioni, dove compare la directory di lavoro sull'host. Chiudere la chat o uscire da PopBot si limita a scollegare: la sessione continua a lavorare sull'host e, alla riapertura, la chat recupera ciò che si è persa. *Arresta sull'host*, nel menu della chat (☰ o clic destro sul chip), la termina; il messaggio successivo avvia lì una nuova sessione che riprende la stessa conversazione. Cambiare modello o impegno fa lo stesso. Gli strumenti che l'host non raggiunge — il server MCP `popbot` di questa macchina e gli MCP editor degli slot — non sono offerti su un host. Fallo girare anche su questo computer (`http://127.0.0.1:7677`) per tenere un agente al lavoro in un checkout che PopBot non gestisce. Anche gli slot funzionano lì: un host tiene un pool di worktree caldi per repository (o un worktree usa e getta per chat), ne prende uno quando una chat viene creata, lo riparcheggia quando la chat viene arrestata sull'host (il lavoro non salvato finisce in uno stash col nome della chat e torna quando la chat riprende) e ricorda chi tiene cosa anche dopo un proprio riavvio. La scheda *Host* nel pannello sinistro mostra i pool di slot di ogni macchina — questo computer per primo — e le chat su ogni host; uno slot occupato o una riga di chat porta la chat in primo piano. Gli host si aggiungono e si rimuovono in Preferenze ▸ Host, dove si gestiscono i repository e i pool di slot di ogni host; questo computer è sempre elencato lì e non si può rimuovere.
...
- L'archivio è ricercabile per nome, ticket, branch e contenuto, e **Cerca nelle chat** (⌘⇧F / Ctrl+Maiusc+F, oppure *Visualizza ▸ Cerca nelle chat…*) cerca in tutte le trascrizioni, aperte e archiviate: le righe trovate sono raggruppate per chat, le migliori prima, e *Vai* apre la chat — riaprendola dall'archivio se serve — e scorre fino a quella riga. I tag di filtro restringono la ricerca — `ticket:ENG-123`, `cr:67`, `last:week`, `from:user`, `tool:Bash`, `agent:codex`, `in:archive`, `chat:login+bug` (nel valore di un tag, `+` unisce parole che devono comparire tutte) — digitati nella casella o scelti con i pulsanti sotto; i soli tag elencano le voci corrispondenti più recenti. Digitando `ticket:`, `cr:` o `chat:` compare sotto la casella l'elenco dei ticket, delle PR e delle chat che PopBot conosce; Tab ne accetta uno; una chat scelta viene inserita come id, così spazi e punteggiatura del titolo non contano.
...
- **Altri agenti** — `send_to_chat` invia un messaggio a un'altra chat e, per impostazione predefinita, attende la fine del turno di quell'agente e ne restituisce la risposta; una chat occupata accoda il messaggio dietro il lavoro in corso. Una chat non può scrivere a sé stessa né chiudersi. Un messaggio inviato così arriva nell'altra chat attribuito alla chat mittente (nome e id) e dice a quell'agente se il mittente attende una risposta o come rispondergli.
...
- **Trascrizioni** — `get_chat_transcript` legge una chat (la propria o un'altra) come voci numerate, per intero o per intervallo; `search_chats` cerca un termine e restituisce il testo attorno a ogni occorrenza, in una chat, in tutte quelle aperte o nell'intero archivio: un indice full-text a trigrammi su ciò che tu e l'agente avete scritto (con `from:tool` anche le chiamate agli strumenti e il loro output), quindi basta un frammento di un identificatore o di un messaggio di errore. La casella di ricerca dell'archivio usa lo stesso indice. `list_refs` fornisce le chiavi dei ticket, i numeri delle PR e gli id delle chat che una ricerca può nominare, e `go_to_message` ti mostra un risultato: porta in primo piano la chat — riaprendola dall'archivio se serve — e scorre fino al messaggio.
...
La striscia stessa mostra tutte le chat aperte in una volta, quante che siano. Le chat attorno a quella che stai guardando compaiono come miniature complete; le altre si comprimono in strisce sottili — l'icona di stato, il nome scritto in verticale, il lampeggio quando lavora — ben serrate perché restino tutte in vista. Scorrendo lateralmente sulla striscia si sposta l'intervallo aperto: da un lato le strisce si allargano in miniature, dall'altro le miniature si stringono in strisce. Un clic su una striscia la apre e porta in primo piano la sua chat, e il trascinamento funziona allo stesso modo per strisce e miniature. Sul terzo superiore della striscia il cursore diventa una mano: premi e trascina lì per far scorrere la striscia a mano.
