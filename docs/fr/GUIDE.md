...
Cinq contrôles de session comptent pour le travail de longue durée :
...
- **Dupliquer la conversation.** Depuis le menu ☰ de la conversation (ou son panneau de réglages), *Dupliquer* démarre une seconde conversation à ce point précis pour que les deux puissent diverger : les échanges sont copiés, la session propre de l'agent est dupliquée (Claude et Codex le prennent en charge nativement, la copie se souvient donc du travail au lieu de le relire), et une conversation avec espace de travail reçoit son propre slot sur une branche `<branch>-fork` reprenant les modifications non validées de l'originale. L'originale continue sans être touchée, et l'agent de la copie apprend où se trouve le dossier d'origine afin de recréer les worktrees propres à la conversation plutôt que de les réutiliser.
- **Exécuter dans le cloud.** À la création d'une conversation, activez la puce *Cloud* à côté du sélecteur d'agent : la conversation s'exécute sur [Anthropic Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview) au lieu d'une CLI locale — une session dans un bac à sable cloud Anthropic, facturée au jeton sur la clé API définie dans Préférences ▸ Agents ▸ Chats cloud, qui continue de travailler après la fermeture de PopBot. Cela fonctionne avec n'importe quel choix d'espace de travail : sans dépôt, le bac à sable est vide ; depuis la racine du dépôt, il clone le dépôt GitHub distant sur la branche où se trouve la racine (poussée d'abord si `origin` ne l'a pas encore) ; avec un slot ou un worktree, la conversation reçoit sa branche comme d'habitude, PopBot la pousse sur `origin` et le bac à sable la clone. Le bac à sable clone avec votre jeton GitHub (celui des Préférences, ou celui avec lequel `gh` est connecté), et l'agent reçoit ce jeton dans le bac à sable pour pouvoir pousser ; il lui est demandé de ne pas le reproduire dans ses réponses, et PopBot ne le stocke jamais dans la conversation. Votre premier message crée la session ; les réponses, appels d'outils et résultats arrivent dans la conversation comme avec n'importe quel agent, et ce qui s'est passé pendant que PopBot était fermé est rejoué à la prochaine ouverture. Les outils s'exécutent dans le bac à sable sans demande de permission (il est isolé ; la seule chose qui en sort est le push vers la branche de la conversation). L'agent commite et pousse sur cette branche ; *Rapatrier le travail du cloud dans le checkout* dans le menu de la conversation (ou ses réglages) fait avancer le checkout local en fast-forward. Changer de modèle ou d'effort termine la session ; le message suivant en démarre une nouvelle, amorcée avec la conversation. Fermer la conversation, la supprimer ou quitter PopBot ne termine jamais la session côté serveur : c'est ainsi qu'elle continue sans vous. *Arrêter le chat cloud* dans le menu de la conversation (☰, ou clic droit sur la puce Cloud) est ce qui la termine : si elle travaille encore, elle est interrompue, puis archivée ; ce qui n'a pas été poussé est perdu, et le message suivant démarre une nouvelle session amorcée avec la conversation. Claude uniquement ; Codex Cloud n'est pas branché.
- **Exécuter sur une autre machine.** *Exécuter sur*, dans le dialogue de nouvelle conversation, liste les hôtes des Préférences ▸ Hôtes : d'autres machines qui font tourner `popbot-host`, un petit démon qui exécute les CLI `claude` et `codex` dans ses propres checkouts. Choisissez-en un et le dialogue montre les dépôts et branches de cet hôte à la place des locaux : la conversation s'exécute à la racine du dépôt, dans un worktree que l'hôte crée sur une nouvelle branche, ou dans un dossier temporaire là-bas sans dépôt. Rien n'est créé sur votre machine ; la transcription, la recherche, les réglages et les règles de permission restent dans la base de PopBot, et les réponses de l'agent, ses appels d'outils et ses demandes de permission arrivent dans la conversation comme d'habitude (les pièces jointes sont envoyées). La puce de la conversation nomme l'hôte et ouvre ses réglages, où figure le répertoire de travail sur l'hôte. Fermer la conversation ou quitter PopBot ne fait que détacher : la session continue sur l'hôte, et la conversation rattrape ce qu'elle a manqué à sa réouverture. *Arrêter sur l'hôte*, dans le menu de la conversation (☰ ou clic droit sur la puce), y met fin ; le message suivant démarre là-bas une nouvelle session qui reprend la même conversation. Changer de modèle ou d'effort fait de même. Les outils que l'hôte ne peut pas atteindre — le serveur MCP `popbot` de cette machine et les MCP d'éditeur des slots — ne sont pas proposés sur un hôte. Faites-le aussi tourner sur cet ordinateur (`http://127.0.0.1:7677`) pour garder un agent au travail dans un checkout que PopBot ne gère pas. Les slots y fonctionnent aussi : un hôte garde un pool de worktrees chauds par dépôt (ou un worktree jetable par conversation), en prend un à la création d'une conversation, le remet au parking quand la conversation est arrêtée sur l'hôte (le travail non validé est mis en stash au nom de la conversation et rejoué à son retour), et se souvient de qui tient quoi à travers ses propres redémarrages. L'onglet *Hôtes* du panneau de gauche montre les pools de slots de chaque machine — cet ordinateur d'abord — et les conversations de chaque hôte ; un slot occupé ou une ligne de conversation met la conversation au premier plan. Les hôtes s'ajoutent et se retirent dans Préférences ▸ Hôtes, où se gèrent les dépôts et pools de slots de chaque hôte ; cet ordinateur y figure toujours et ne peut pas être retiré.
...
- L'archive est consultable par nom, ticket, branche et contenu, et **Rechercher dans les conversations** (⌘⇧F / Ctrl+Maj+F, ou *Affichage ▸ Rechercher dans les conversations…*) cherche dans toutes les transcriptions, ouvertes comme archivées : les lignes trouvées sont regroupées par conversation, les meilleures d'abord, et *Aller* ouvre la conversation — en la rouvrant depuis l'archive si besoin — et fait défiler jusqu'à cette ligne. Des tags de filtre affinent la recherche — `ticket:ENG-123`, `cr:67`, `last:week`, `from:user`, `tool:Bash`, `agent:codex`, `in:archive`, `chat:login+bug` (dans la valeur d'un tag, `+` relie des mots qui doivent tous apparaître) — tapés dans le champ ou choisis via les boutons en dessous ; des tags seuls listent les entrées correspondantes les plus récentes. Taper `ticket:`, `cr:` ou `chat:` propose, dans une liste sous le champ, les tickets, PR et conversations que PopBot connaît ; Tab en accepte un ; une conversation choisie est insérée sous forme d'identifiant, si bien que les espaces et la ponctuation du titre n'ont aucune importance.
...
- **Autres agents** — `send_to_chat` envoie un message à une autre conversation et, par défaut, attend la fin du tour de cet agent pour renvoyer sa réponse ; une conversation occupée met le message en file derrière son travail en cours. Une conversation ne peut ni s'écrire ni se fermer elle-même. Un message envoyé ainsi arrive dans l'autre conversation attribué à la conversation émettrice (son nom et son id) et indique à cet agent si l'expéditeur attend une réponse ou comment lui répondre.
...
- **Transcriptions** — `get_chat_transcript` lit une conversation (la sienne ou une autre) sous forme d'entrées numérotées, en entier ou par plage ; `search_chats` cherche un terme et renvoie le texte autour de chaque occurrence, dans une conversation, dans toutes celles ouvertes ou dans toute l'archive : un index plein texte par trigrammes sur ce que vous et l'agent avez écrit (avec `from:tool`, aussi les appels d'outils et leur sortie), si bien qu'un fragment d'identifiant ou de message d'erreur suffit. Le champ de recherche de l'archive utilise le même index. `list_refs` fournit les clés de ticket, numéros de PR et identifiants de conversation qu'une recherche peut nommer, et `go_to_message` vous montre un résultat : il met la conversation au premier plan — en la rouvrant depuis l'archive si besoin — et fait défiler jusqu'au message.
