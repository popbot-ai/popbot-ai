...
- **Exécuter dans le cloud.** À la création d'une conversation, choisissez *Exécuter dans le cloud* à la place d'une branche de base : la conversation pilote alors une [session cloud de Claude Code](https://claude.ai/code) au lieu d'un agent local. Votre premier message démarre la session depuis le terminal de la conversation (`claude --cloud`, à partir du dépôt distant GitHub sur la branche où se trouve la racine du dépôt, poussez donc d'abord), les messages suivants y sont mis en file comme relances, et la session continue de tourner après la fermeture de PopBot. La pastille de l'en-tête renvoie à la session sur claude.ai, et *Téléporter dans le terminal* dans les réglages de la conversation ramène sa branche et sa conversation sur votre machine. Claude uniquement pour l'instant ; Codex Cloud n'est pas branché.
