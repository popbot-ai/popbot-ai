...
- **Exécuter dans le cloud.** À la création d'une conversation, activez le bouton *Cloud* à côté du sélecteur d'agent : la conversation pilote alors une [session cloud de Claude Code](https://claude.ai/code) au lieu d'un agent local. Cela fonctionne quel que soit l'espace de travail choisi : sans dépôt, la session tourne simplement à distance ; depuis la racine du dépôt, le cloud clone le dépôt distant GitHub sur la branche où se trouve la racine (poussez-la d'abord) ; avec un slot ou un worktree, la conversation reçoit sa branche comme d'habitude, PopBot la pousse vers `origin` et le cloud clone cette branche tandis que le slot reste local pour téléporter le travail en retour. Votre premier message démarre la session depuis le terminal de la conversation (`claude --cloud`) et, tant que cette CLI reste attachée, la conversation est reflétée dans la colonne de la conversation — réponses, appels d'outils et le reste — et les messages que vous envoyez y sont tapés directement. Une fois la CLI terminée, les messages sont encore mis en file dans la session (`claude -p --cloud`), mais les réponses ne sont visibles que sur claude.ai. La session continue de tourner après la fermeture de PopBot. Le cloud accède à GitHub via votre compte claude.ai, pas via votre machine : installez la Claude GitHub App depuis claude.ai/code, ou lancez `/web-setup` une fois dans une session `claude` du terminal. La pastille de l'en-tête renvoie à la session sur claude.ai, et *Téléporter dans le terminal* dans les réglages de la conversation ramène sa branche et sa conversation sur votre machine. Claude uniquement pour l'instant ; Codex Cloud n'est pas branché.
