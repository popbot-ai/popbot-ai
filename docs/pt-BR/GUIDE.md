...
Cinco controles de sessão importam para trabalho de longa duração:
...
- **Compactar o contexto.** O medidor circular na área de composição mostra o quanto a janela de contexto do agente está cheia. Clique (ou clique com o botão direito) e escolha *Compactar contexto* para que o Claude resuma a conversa até aqui; a transcrição registra quando termina e quanto espaço liberou. Digitar `/compact` faz o mesmo. O Codex compacta por conta própria, então ali o medidor é apenas informativo.
- **Bifurcar o chat.** No menu ☰ do chat (ou na folha de configurações), *Bifurcar chat* inicia um segundo chat exatamente neste ponto para que os dois possam divergir: a conversa até aqui é copiada, a sessão do próprio agente é bifurcada (Claude e Codex suportam isso nativamente, então a bifurcação lembra do trabalho em vez de relê-lo) e um chat com espaço de trabalho recebe seu próprio slot novo em um branch `<branch>-fork` com as alterações não confirmadas do original. O original continua intacto, e o agente da bifurcação é informado de onde fica a pasta de origem do original para recriar worktrees específicos do chat em vez de reutilizá-los.
- **Executar na nuvem.** Ao criar um chat, ligue o chip *Cloud* ao lado do seletor de agente e o chat roda no [Anthropic Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview) em vez de em uma CLI local: uma sessão em um sandbox na nuvem da Anthropic, cobrada por token na chave de API definida em Preferências ▸ Agentes ▸ Chats na nuvem, que continua trabalhando depois que o PopBot fecha. Funciona com qualquer escolha de workspace: sem repositório, o sandbox fica vazio; a partir da raiz do repositório, o sandbox clona o remoto GitHub no branch em que a raiz está (com push antes, se `origin` ainda não o tiver); com um slot ou worktree, o chat recebe seu branch como sempre, o PopBot o envia para `origin` e o sandbox clona esse branch. O sandbox clona com o seu token do GitHub (o das Preferências, ou aquele com que o `gh` está autenticado), e o agente recebe esse token dentro do sandbox para poder fazer push; ele é instruído a mantê-lo fora das respostas, e o PopBot nunca o armazena no chat. Sua primeira mensagem cria a sessão; respostas, chamadas de ferramentas e resultados chegam ao chat como com qualquer outro agente, e o que aconteceu enquanto o PopBot estava fechado é reproduzido na próxima vez que o chat é aberto. As ferramentas rodam no sandbox sem pedidos de permissão (ele é isolado; a única coisa que sai dele é o push para o branch do chat). O agente faz commit e push nesse branch; *Trazer o trabalho da nuvem para o checkout* no menu do chat (ou nas configurações do chat) faz fast-forward do checkout local até ele. Mudar o modelo ou o esforço encerra a sessão; a próxima mensagem inicia outra, preparada com a conversa. Fechar o chat, excluí-lo ou sair do PopBot nunca encerra a sessão no servidor: é assim que ela continua trabalhando sem você. *Desligar o chat na nuvem* no menu do chat (☰, ou clique com o botão direito no chip Cloud) é o que a encerra: se ainda estiver trabalhando, ela é interrompida e depois arquivada; o que não foi enviado é perdido, e a próxima mensagem inicia uma nova sessão preparada com a conversa. Somente Claude; o Codex Cloud não está conectado.
- **Executar em outra máquina.** *Executar em*, no diálogo de novo chat, lista os hosts de Preferências ▸ Hosts: outras máquinas rodando o `popbot-host`, um pequeno daemon que executa as CLIs `claude` e `codex` em checkouts próprios. Escolha um e o diálogo mostra os repositórios e branches desse host em vez dos locais: o chat roda na raiz do repositório, num worktree que o host cria num branch novo, ou numa pasta temporária lá sem repositório. Nada é criado na sua máquina; a transcrição, a busca, as configurações e as regras de permissão ficam no banco de dados do PopBot, e as respostas do agente, chamadas de ferramentas e pedidos de permissão chegam ao chat como sempre (os anexos são enviados). O chip do chat nomeia o host e abre suas configurações, onde aparece o diretório de trabalho no host. Fechar o chat ou sair do PopBot apenas desconecta: a sessão continua trabalhando no host e, ao reabrir, o chat recupera o que perdeu. *Encerrar no host*, no menu do chat (☰ ou clique direito no chip), termina a sessão; a próxima mensagem inicia lá uma nova sessão que retoma a mesma conversa. Mudar o modelo ou o esforço faz o mesmo. Ferramentas que o host não alcança — o servidor MCP `popbot` desta máquina e os MCPs de editor dos slots — não são oferecidas num host. Rode-o também neste computador (`http://127.0.0.1:7677`) para manter um agente trabalhando num checkout que o PopBot não gerencia. Slots também funcionam lá: um host mantém um pool de worktrees aquecidos por repositório (ou um worktree descartável por chat), pega um quando um chat é criado, o estaciona de novo quando o chat é encerrado no host (o trabalho não salvo vai para um stash com o nome do chat e volta quando o chat retorna) e lembra quem segura o quê mesmo após reiniciar. A aba *Hosts* no painel esquerdo mostra os pools de slots de cada máquina — este computador primeiro — e os chats em cada host; um slot ocupado ou uma linha de chat foca o chat. Hosts são adicionados e removidos em Preferências ▸ Hosts, onde os repositórios e pools de slots de cada host são gerenciados; este computador sempre aparece lá e não pode ser removido.
...
- O arquivo é pesquisável por nome, ticket, branch e conteúdo, e **Pesquisar nos chats** (⌘⇧F / Ctrl+Shift+F, ou *Exibir ▸ Pesquisar nos chats…*) pesquisa todas as transcrições, abertas e arquivadas: as linhas encontradas aparecem por chat, as melhores primeiro, e *Ir* abre o chat — reabrindo-o do arquivo se preciso — e rola até essa linha. Tags de filtro restringem a busca — `ticket:ENG-123`, `cr:67`, `last:week`, `from:user`, `tool:Bash`, `agent:codex`, `in:archive`, `chat:login+bug` (no valor de uma tag, `+` une palavras que devem aparecer todas) — digitadas na caixa ou escolhidas nos botões abaixo dela; só com tags, listam-se as entradas correspondentes mais recentes. Ao digitar `ticket:`, `cr:` ou `chat:`, uma lista com os tickets, PRs e chats que o PopBot conhece aparece sob a caixa; Tab aceita um; um chat escolhido é inserido como seu id, então espaços e pontuação do título não importam.
...
### Ferramentas do PopBot para agentes

O agente de cada chat — Claude ou Codex — recebe um servidor MCP `popbot`, para que um agente possa operar o app como você e os chats possam conversar entre si:
...
- **Outros agentes** — `send_to_chat` envia uma mensagem a outro chat e, por padrão, espera o turno daquele agente terminar e devolve a resposta; um chat ocupado recebe a mensagem na fila, atrás do trabalho atual. Um chat não pode enviar mensagem para si mesmo nem se fechar. Uma mensagem enviada assim chega ao outro chat atribuída ao chat remetente (nome e id) e diz àquele agente se o remetente está aguardando resposta ou como respondê-lo.
...
- **Transcrições** — `get_chat_transcript` lê um chat (o próprio ou outro) como entradas numeradas, inteiro ou por faixa; `search_chats` encontra um termo com o texto ao redor de cada ocorrência, em um chat, em todos os abertos ou em todo o arquivo: um índice de texto completo por trigramas sobre o que você e o agente escreveram (com `from:tool`, também as chamadas de ferramentas e a saída delas), então um fragmento de um identificador ou de uma mensagem de erro basta. A caixa de busca do arquivo usa o mesmo índice. `list_refs` dá as chaves de ticket, números de PR e ids de chat que uma busca pode nomear, e `go_to_message` mostra um resultado para você: foca o chat — reabrindo-o do arquivo se preciso — e rola até a mensagem.
