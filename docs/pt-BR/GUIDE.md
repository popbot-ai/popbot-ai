...
- **Executar na nuvem.** Ao criar um chat, ligue o botão *Nuvem* ao lado do seletor de agente e o chat conduzirá uma [sessão na nuvem do Claude Code](https://claude.ai/code) em vez de um agente local: funciona com qualquer escolha de espaço de trabalho: sem repositório, a sessão simplesmente roda remotamente; a partir da raiz do repositório, a nuvem clona o remoto GitHub no branch em que a raiz está (faça push primeiro); com um slot ou worktree, o chat recebe seu branch como sempre, o PopBot o envia ao `origin` e a nuvem clona esse branch enquanto o slot fica local para teletransportar o trabalho de volta. Sua primeira mensagem inicia a sessão a partir do terminal do chat (`claude --cloud`) e, enquanto essa CLI permanece conectada, a conversa é espelhada na coluna do chat — respostas, chamadas de ferramentas e tudo — e as mensagens que você envia são digitadas direto nela. Depois que a CLI encerra, as mensagens ainda entram na fila da sessão (`claude -p --cloud`), mas as respostas ficam só no claude.ai. A sessão continua rodando depois que o PopBot é fechado. A nuvem acessa o GitHub pela sua conta claude.ai, não pela sua máquina: instale o Claude GitHub App em claude.ai/code ou rode `/web-setup` uma vez numa sessão `claude` no terminal. O chip do cabeçalho leva à sessão no claude.ai, e *Teletransportar para o terminal* nas configurações do chat traz o branch e a conversa de volta para a sua máquina. Só Claude por enquanto; o Codex Cloud não está conectado.
