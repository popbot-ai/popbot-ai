...
- **Executar na nuvem.** Ao criar um chat, escolha *Executar na nuvem* no lugar de um branch base e o chat conduzirá uma [sessão na nuvem do Claude Code](https://claude.ai/code) em vez de um agente local: sua primeira mensagem inicia a sessão a partir do terminal do chat (`claude --cloud`, usando o remoto GitHub do repositório no branch em que a raiz do repo está, então faça push primeiro), as mensagens seguintes entram na fila como acompanhamentos e a sessão continua rodando depois que o PopBot é fechado. O chip do cabeçalho leva à sessão no claude.ai, e *Teletransportar para o terminal* nas configurações do chat traz o branch e a conversa de volta para a sua máquina. Só Claude por enquanto; o Codex Cloud não está conectado.
