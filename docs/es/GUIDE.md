...
- **Ejecutar en la nube.** Al crear un chat, activa el botón *Nube* junto al selector de agente y el chat dirigirá una [sesión en la nube de Claude Code](https://claude.ai/code) en vez de un agente local: funciona con cualquier elección de espacio de trabajo: sin repositorio, la sesión simplemente se ejecuta en remoto; desde la raíz del repositorio, la nube clona el remoto de GitHub en la rama en la que está la raíz (haz push primero); con un slot o worktree, el chat recibe su rama como siempre, PopBot la sube a `origin` y la nube clona esa rama mientras el slot sigue en local para teletransportar el trabajo de vuelta. Tu primer mensaje inicia la sesión desde el terminal del chat (`claude --cloud`) y, mientras esa CLI siga conectada, la conversación se refleja en la columna del chat —respuestas, llamadas a herramientas y todo— y los mensajes que envías se escriben directamente en ella. Cuando la CLI ha terminado, los mensajes siguen encolándose en la sesión (`claude -p --cloud`), pero las respuestas solo están en claude.ai. La sesión sigue ejecutándose cuando PopBot se cierra. La nube accede a GitHub con tu cuenta de claude.ai, no con tu equipo: instala la Claude GitHub App desde claude.ai/code o ejecuta `/web-setup` una vez en una sesión de `claude` en el terminal. El chip de la cabecera enlaza a la sesión en claude.ai, y *Teletransportar al terminal* en los ajustes del chat trae su rama y su conversación de vuelta a tu máquina. Solo Claude por ahora; Codex Cloud no está conectado.
