...
Cinco controles de sesión importan para el trabajo de larga duración:
...
- **Bifurcar el chat.** Desde el menú ☰ del chat (o su hoja de ajustes), *Bifurcar chat* inicia un segundo chat en este punto exacto para que ambos puedan divergir: se copia la conversación hasta ahora, se bifurca la propia sesión del agente (Claude y Codex lo admiten de forma nativa, así que la bifurcación recuerda el trabajo en lugar de releerlo) y un chat con espacio de trabajo recibe su propio slot nuevo en una rama `<branch>-fork` con los cambios sin confirmar del original. El original sigue intacto, y al agente de la bifurcación se le indica dónde está la carpeta base del original para que recree los worktrees específicos del chat en lugar de reutilizarlos.
- **Ejecutar en la nube.** Al crear un chat, activa el chip *Cloud* junto al selector de agente y el chat se ejecuta en [Anthropic Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview) en lugar de en una CLI local: una sesión en un sandbox en la nube de Anthropic, facturada por token a la clave de API configurada en Preferencias ▸ Agentes ▸ Chats en la nube, que sigue trabajando cuando PopBot se cierra. Funciona con cualquier elección de espacio de trabajo: sin repo, el sandbox está vacío; desde la raíz del repo, el sandbox clona el remoto de GitHub en la rama en la que está la raíz (se hace push antes si `origin` aún no la tiene); con un slot o worktree, el chat recibe su rama como siempre, PopBot la sube a `origin` y el sandbox la clona. El sandbox clona con tu token de GitHub (el de Preferencias, o aquel con el que `gh` tiene la sesión iniciada), y el agente recibe ese token dentro del sandbox para poder hacer push; se le indica que lo mantenga fuera de sus respuestas, y PopBot nunca lo guarda en el chat. Tu primer mensaje crea la sesión; las respuestas, las llamadas a herramientas y sus resultados llegan al chat como con cualquier otro agente, y lo ocurrido mientras PopBot estaba cerrado se reproduce al abrir el chat de nuevo. Las herramientas se ejecutan en el sandbox sin pedir permiso (está aislado; lo único que sale de él es el push a la rama del chat). El agente hace commit y push a esa rama; *Traer el trabajo de la nube al checkout* en el menú del chat (o en los ajustes del chat) hace fast-forward del checkout local hasta ella. Cambiar el modelo o el esfuerzo termina la sesión; el siguiente mensaje inicia otra preparada con la conversación. Cerrar el chat, eliminarlo o salir de PopBot nunca termina la sesión en el servidor: así es como sigue trabajando sin ti. *Apagar el chat en la nube* en el menú del chat (☰, o clic derecho en el chip Cloud) es lo que la termina: si aún está trabajando se interrumpe y luego se archiva; lo no subido se pierde, y el siguiente mensaje inicia una sesión nueva preparada con la conversación. Solo Claude; Codex Cloud no está conectado.
- **Ejecutar en otro equipo.** *Ejecutar en*, en el diálogo de nuevo chat, lista los hosts de Preferencias ▸ Hosts: otras máquinas que ejecutan `popbot-host`, un pequeño demonio que corre las CLI `claude` y `codex` en sus propios checkouts. Elige uno y el diálogo muestra los repositorios y ramas de ese host en lugar de los locales: el chat se ejecuta en la raíz del repositorio, en un worktree que el host crea en una rama nueva, o en una carpeta temporal allí sin repositorio. En tu máquina no se crea nada; la transcripción, la búsqueda, los ajustes y las reglas de permisos se quedan en la base de datos de PopBot, y las respuestas del agente, sus llamadas a herramientas y sus peticiones de permiso llegan al chat como siempre (los adjuntos se envían). El chip del chat nombra al host y abre sus ajustes, donde se ve el directorio de trabajo en el host. Cerrar el chat o salir de PopBot solo desconecta: la sesión sigue trabajando en el host y, al reabrir el chat, este se pone al día con lo que se perdió. *Apagar en el host*, en el menú del chat (☰ o clic derecho en el chip), la termina; el siguiente mensaje inicia allí una sesión nueva que retoma la misma conversación. Cambiar el modelo o el esfuerzo hace lo mismo. Las herramientas que el host no alcanza —el servidor MCP `popbot` de esta máquina y los MCP de editor de los slots— no se ofrecen en un host. Ejecútalo también en este equipo (`http://127.0.0.1:7677`) para mantener a un agente trabajando en un checkout que PopBot no gestiona. Los slots también funcionan allí: un host mantiene un pool de worktrees calientes por repositorio (o un worktree desechable por chat), toma uno cuando se crea un chat, lo aparca de nuevo cuando el chat se apaga en el host (el trabajo sin guardar se guarda en un stash con el nombre del chat y se recupera al volver) y recuerda quién tiene qué aunque se reinicie. La pestaña *Hosts* del panel izquierdo muestra los pools de slots de cada máquina —este equipo primero— y los chats de cada host; un slot ocupado o una fila de chat enfoca el chat. Los hosts se añaden y se quitan en Preferencias ▸ Hosts, donde se gestionan los repositorios y los pools de slots de cada host; este equipo siempre aparece ahí y no se puede quitar.
...
- El archivo se puede buscar por nombre, ticket, rama y contenido, y **Buscar en los chats** (⌘⇧F / Ctrl+Mayús+F, o *Ver ▸ Buscar en los chats…*) busca en todas las transcripciones, abiertas y archivadas: las líneas coincidentes se muestran por chat, las mejores primero, e *Ir* abre el chat —reabriéndolo desde el archivo si hace falta— y se desplaza hasta esa línea. Las etiquetas de filtro acotan la búsqueda —`ticket:ENG-123`, `cr:67`, `last:week`, `from:user`, `tool:Bash`, `agent:codex`, `in:archive`, `chat:login+bug` (en el valor de una etiqueta, `+` une palabras que deben aparecer todas)—, escritas en el cuadro o elegidas con los botones de debajo; solo con etiquetas se listan las entradas coincidentes más recientes. Al escribir `ticket:`, `cr:` o `chat:` aparece bajo el cuadro una lista de los tickets, PRs y chats que PopBot conoce; Tab acepta uno; un chat elegido se inserta como su id, así que los espacios y signos del título no importan.
...
- **Otros agentes** — `send_to_chat` envía un mensaje a otro chat y, por defecto, espera a que termine el turno de ese agente y devuelve su respuesta; si el chat está ocupado, el mensaje se encola tras su trabajo actual. Un chat no puede escribirse ni cerrarse a sí mismo. Un mensaje enviado así llega al otro chat atribuido al chat remitente (su nombre y su id) e indica a ese agente si el remitente espera respuesta o cómo contestarle.
...
- **Transcripciones** — `get_chat_transcript` lee un chat (el propio u otro) como entradas numeradas, entero o por rango; `search_chats` busca un término y muestra el texto alrededor de cada coincidencia, en un chat, en todos los abiertos o en todo el archivo: un índice de texto completo por trigramas sobre lo que escribisteis tú y el agente (con `from:tool`, también las llamadas a herramientas y su salida), así que basta con un fragmento de un identificador o de un mensaje de error. El buscador del archivo usa el mismo índice. `list_refs` da las claves de ticket, números de PR e ids de chat que una búsqueda puede nombrar, y `go_to_message` te muestra una coincidencia: enfoca el chat —reabriéndolo desde el archivo si hace falta— y se desplaza hasta el mensaje.
