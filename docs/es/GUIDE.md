...
- **Transcripciones** — `get_chat_transcript` lee un chat (el propio u otro) como entradas numeradas, entero o por rango; `search_chats` busca un término y muestra el texto alrededor de cada coincidencia, en un chat, en todos los abiertos o en todo el archivo: un índice de texto completo por trigramas sobre cada mensaje, llamada a herramienta y resultado, así que basta con un fragmento de un identificador o de un mensaje de error. El buscador del archivo usa el mismo índice.
