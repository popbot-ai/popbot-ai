...
- **Ejecutar en la nube.** Al crear un chat, elige *Ejecutar en la nube* en lugar de una rama base y el chat dirigirá una [sesión en la nube de Claude Code](https://claude.ai/code) en vez de un agente local: tu primer mensaje inicia la sesión desde el terminal del chat (`claude --cloud`, usando el remoto de GitHub del repositorio en la rama en la que está la raíz del repo, así que haz push primero), los mensajes posteriores se encolan como seguimientos y la sesión sigue ejecutándose cuando PopBot se cierra. El chip de la cabecera enlaza a la sesión en claude.ai, y *Teletransportar al terminal* en los ajustes del chat trae su rama y su conversación de vuelta a tu máquina. Solo Claude por ahora; Codex Cloud no está conectado.
