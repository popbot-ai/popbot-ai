...
**클라우드 채팅** — 클라우드 채팅이 사용하는 *Anthropic API 키*(로컬 저장, 없으면 환경 변수 `ANTHROPIC_API_KEY` 사용)와 샌드박스가 리포지토리를 클론할 때 쓰는 *GitHub 토큰*(`repo` 범위, 없으면 `gh auth token` 사용). 리포지토리가 있는 클라우드 채팅에는 둘 다 필요하고, 없는 채팅에는 키만 필요합니다. 키를 넣고 저장하면 먼저 API로 검증합니다. 클라우드 세션은 Claude 구독이 아니라 키의 Console 계정에 토큰 단위로 과금됩니다. 처음 사용할 때 PopBot은 그 계정에 환경 하나와 모델·추론 강도별 에이전트 하나를 만들고 이후 재사용합니다([클라우드에서 실행](GUIDE.md#chats) 참고).
