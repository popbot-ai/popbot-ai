...
PopBot의 모든 것은 앱 안의 **환경설정**(제목 표시줄의 톱니바퀴, 또는 `⌘,`)을 통해 설정됩니다 — 손으로 편집해야 할 설정 파일은 없습니다. 이 가이드는 내비게이션이 나열하는 순서대로 모든 패널을 살펴보며, 이는 대체로 처음 설정할 때의 순서와 같습니다. 어디에도 저장 버튼이 없습니다. 필드는 벗어나는 순간(또는 Enter를 누르는 순간), 스위치는 전환하는 순간 저장되며, 환경설정을 닫으면 아직 남아 있는 것도 저장됩니다.
...
자격 증명이 있는 각 트래커는 자격 증명 필드를 벗어나는 즉시 검증하고, 결과와 관계없이 저장하며, *연결됨 / 연결되지 않음* 상태 알약으로 결과를 보여줍니다.
...
**작업 중인 Codex에 지시 전달** *(기본값 꺼짐)* — exec SDK 대신 Codex의 `app-server`로 연결합니다. 켜면 Codex가 작업하는 동안 보낸 메시지가 실행 중인 턴에 합쳐져, 턴이 끝나길 기다리지 않고 모델의 다음 단계(보통 실행 중인 명령이 끝나는 순간)에 전달됩니다. Codex 채팅의 컨텍스트 게이지도 채워지고, **컨텍스트 압축**을 사용할 수 있으며, **Stop**도 턴을 깔끔하게 중단합니다. Codex는 `app-server`를 실험적 기능으로 표시하고 `codex` 0.153 이상이 필요하므로 선택 사항입니다. 이전 CLI에서는 그냥 대기열 방식으로 돌아갑니다. 설정은 각 채팅의 다음 메시지부터 적용되며, 채팅은 스레드를 잃지 않고 두 연결 사이를 오갈 수 있습니다.

**에이전트용 PopBot 도구** *(기본 켜짐)* — 각 채팅의 에이전트에게 `popbot` MCP 서버를 제공합니다. 채팅 목록 조회·생성·닫기·다시 열기, 다른 채팅에 메시지를 보내고 답 기다리기, 코드 리뷰와 티켓 채팅 시작, 기록 읽기와 검색 도구가 있습니다([에이전트용 PopBot 도구](GUIDE.md#에이전트용-popbot-도구) 참고). 서버는 localhost에서만, 무작위 포트와 실행마다 새로 만드는 비밀값이 URL에 포함된 채로 대기합니다. 끄면 각 채팅의 다음 에이전트 세션부터 적용됩니다.

**클라우드 채팅** — 클라우드 채팅이 사용하는 *Anthropic API 키*(로컬 저장, 없으면 환경 변수 `ANTHROPIC_API_KEY` 사용)와 샌드박스가 리포지토리를 클론할 때 쓰는 *GitHub 토큰*(`repo` 범위, 없으면 `gh auth token` 사용). 리포지토리가 있는 클라우드 채팅에는 둘 다 필요하고, 없는 채팅에는 키만 필요합니다. 키를 넣고 저장하면 먼저 API로 검증합니다. 클라우드 세션은 Claude 구독이 아니라 키의 Console 계정에 토큰 단위로 과금됩니다. 처음 사용할 때 PopBot은 그 계정에 환경 하나와 모델·추론 강도별 에이전트 하나를 만들고 이후 재사용합니다([클라우드에서 실행](GUIDE.md#chats) 참고). 조직 수준 키(워크스페이스 안에서 만들지 않은 키)에는 *워크스페이스 ID*(`wrkspc_…`, Console의 Workspaces 페이지)도 필요하며, 없으면 키 검사가 알려줍니다.

## 호스트

이 PopBot의 채팅을 실행하는 다른 컴퓨터입니다. 각 컴퓨터는 이 저장소에서 빌드한 단일 파일 Node 데몬 `popbot-host`를 실행합니다.

```sh
npm run build:host                       # dist-host/popbot-host.cjs 생성
node dist-host/popbot-host.cjs --init --repo popbot=/path/to/checkout
node dist-host/popbot-host.cjs           # 127.0.0.1:7677에서 대기
```

`--init`은 `~/.popbot-host/config.json`(바인드 주소, 포트, 무작위 베어러 토큰, 작업 공간 폴더, id와 경로로 된 저장소 목록)을 기록하고 토큰을 출력합니다. 파일을 편집하거나 `--port`, `--bind`, `--token`, `--name`, `--workspaces`와 추가 `--repo id=/path` 플래그를 전달하세요. 호스트에는 Node 20 이상과 PATH에 있는 `claude` 및/또는 `codex` CLI가 필요합니다. 이 컴퓨터에서 아무것도 복사하지 않고 대화 기록도 보관하지 않습니다. 실행 중인 세션과 그 이벤트 로그만 유지하며, 다시 연결한 PopBot은 중단된 지점부터 재생합니다. localhost에 바인드하므로 원격 컴퓨터는 포트를 노출하지 말고 SSH 터널(`ssh -L 7677:127.0.0.1:7677 machine`)로 연결하세요.

또는 아무것도 설치하지 않고 공개 이미지를 실행할 수 있습니다. `ghcr.io/popbot-ai/popbot-host`(버전 태그마다 *Host image* 워크플로가 빌드)에는 Node, git, 두 CLI와 데몬이 들어 있습니다. `docker/host/docker-compose.yml`이 바로 쓸 수 있는 예시입니다. 체크아웃을 `/repos`에, 설정·작업 공간·로그용 볼륨을 `/data`에, `claude`와 `codex login`을 한 번 실행한 사용자의 로그인 정보를 `/home/popbot/.claude`와 `/home/popbot/.codex`에 마운트하세요(Mac에서는 `security find-generic-password -s "Claude Code-credentials" -w`로 키체인의 Claude 로그인을 `.claude/.credentials.json`으로 내보냅니다). 컨테이너 명령으로 `--repo id=/repos/x`와 `--slots x=4`를 전달하고, 포트 7677을 localhost나 테일넷 주소에 공개하며, 토큰은 컨테이너의 첫 로그 줄에서 읽습니다(또는 `POPBOT_HOST_TOKEN`을 설정).

PopBot에서 *호스트 추가*는 로컬 주소가 채워진 항목을 만듭니다. 이름, URL, 토큰을 설정하면(필드는 벗어날 때 저장됩니다) 패널이 호스트에 버전, Claude와 Codex를 찾았는지, 저장소 목록을 묻습니다. 호스트의 워크트리는 작업 공간 폴더(`~/.popbot-host/workspaces/<repo>/<branch>`) 아래에 있고, 보낸 첨부 파일은 그곳의 `attachments/<채팅 id>`에 저장됩니다. 권한 규칙은 요청마다 함께 전달되므로 *항상 허용* 결정은 호스트에서도 같은 방식으로 적용됩니다. 호스트를 제거해도 이미 그 위에 있는 채팅은 목록에 남지만 더 이상 연결할 수 없습니다. [다른 컴퓨터에서 실행](GUIDE.md#chats)을 참고하세요.

이 컴퓨터는 항상 첫 번째 항목이며 제거할 수 없습니다. 그 저장소와 슬롯 풀은 저장소 섹션입니다. 다른 각 호스트의 저장소는 해당 카드에서 편집합니다. 호스트의 경로, 기준 브랜치, 슬롯 풀(접두사와 개수, 또는 채팅마다 워크트리를 두는 *임시*)을 바꾸면 호스트가 설정을 다시 씁니다(명령줄의 `--slots id=N` 또는 `--slots id=ephemeral`도 같은 일을 하며, 새 저장소는 id 이름의 슬롯 4개를 받습니다). 슬롯 워크트리는 `<workspaces>/<repo>/<prefix>-N`에 있고 비어 있을 때는 `<repo>/slotN`에 대기합니다. 누가 무엇을 잡고 있는지는 `<workspaces>/state.json`에 저장되어 호스트 재시작 후에도 유지됩니다.
