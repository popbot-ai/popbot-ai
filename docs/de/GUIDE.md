...
- Das Archiv ist durchsuchbar nach Name, Ticket, Branch und Inhalt, und **Chats durchsuchen** (⌘⇧F / Strg+Umschalt+F oder *Ansicht ▸ Chats durchsuchen…*) durchsucht jedes Transkript, offen wie archiviert: passende Zeilen erscheinen je Chat, beste zuerst, und *Gehe zu* öffnet den Chat – bei Bedarf aus dem Archiv – und scrollt zu dieser Zeile. Filter-Tags grenzen die Suche ein – `ticket:ENG-123`, `cr:67`, `last:week`, `from:user`, `tool:Bash`, `agent:codex`, `in:archive`, `chat:name` – getippt oder über die Buttons unter dem Feld gewählt; Tags allein listen die neuesten passenden Einträge. Beim Tippen von `ticket:`, `cr:` oder `chat:` erscheint unter dem Feld eine Liste der Tickets, PRs und Chats, die PopBot kennt; Tab übernimmt einen Eintrag.
