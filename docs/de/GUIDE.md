...
- **In der Cloud ausführen.** Schalte beim Anlegen eines Chats den *Cloud*-Button neben der Agentenauswahl ein, und der Chat steuert eine [Claude-Code-Cloud-Session](https://claude.ai/code) statt eines lokalen Agenten: das geht mit jeder Arbeitsbereichswahl: ohne Repo läuft die Session einfach remote; aus dem Repo-Root klont die Cloud das GitHub-Remote auf dem Branch, auf dem der Root steht (vorher pushen); mit Slot oder Worktree bekommt der Chat wie üblich seinen Branch, PopBot pusht ihn nach `origin`, und die Cloud klont diesen Branch, während der Slot lokal bleibt, um die Arbeit zurückzuteleportieren. Deine erste Nachricht startet die Session aus dem Terminal des Chats (`claude --cloud`), und solange diese CLI verbunden bleibt, wird das Gespräch in die Chat-Spalte gespiegelt – Antworten, Tool-Aufrufe und alles – und Nachrichten, die du sendest, werden direkt hineingetippt. Ist die CLI beendet, werden Nachrichten weiterhin in die Session eingereiht (`claude -p --cloud`), die Antworten gibt es dann aber nur auf claude.ai. Die Session läuft weiter, wenn PopBot beendet wird. Die Cloud erreicht GitHub über dein claude.ai-Konto, nicht über deinen Rechner: Installiere die Claude GitHub App von claude.ai/code oder führe `/web-setup` einmal in einer `claude`-Terminalsitzung aus. Der Chip im Header verlinkt auf die Session auf claude.ai, und *Ins Terminal teleportieren* in den Chat-Einstellungen holt Branch und Gespräch zurück auf deinen Rechner. Vorerst nur Claude; Codex Cloud ist nicht angebunden.
