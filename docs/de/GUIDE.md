...
- **In der Cloud ausführen.** Wähle beim Anlegen eines Chats *In der Cloud ausführen* statt eines Basis-Branches, und der Chat steuert eine [Claude-Code-Cloud-Session](https://claude.ai/code) statt eines lokalen Agenten: deine erste Nachricht startet die Session aus dem Terminal des Chats (`claude --cloud`, mit dem GitHub-Remote des Repos auf dem Branch, auf dem der Repo-Root steht – also vorher pushen), spätere Nachrichten werden als Nachfragen eingereiht, und die Session läuft weiter, wenn PopBot beendet wird. Der Chip im Header verlinkt auf die Session auf claude.ai, und *Ins Terminal teleportieren* in den Chat-Einstellungen holt Branch und Gespräch zurück auf deinen Rechner. Vorerst nur Claude; Codex Cloud ist nicht angebunden.
