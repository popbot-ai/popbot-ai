...
Fünf Session-Steuerelemente sind für lang laufende Arbeit wichtig:
...
- **Chat abzweigen.** *Chat abzweigen* im ☰-Menü des Chats (oder in seinem Einstellungsblatt) startet an genau dieser Stelle einen zweiten Chat, damit beide auseinanderlaufen können: das bisherige Gespräch wird kopiert, die eigene Session des Agenten wird abgezweigt (Claude und Codex unterstützen das nativ, die Abzweigung erinnert sich also an die Arbeit, statt sie neu zu lesen), und ein Chat mit Arbeitsbereich bekommt einen eigenen frischen Slot auf einem `<branch>-fork`-Branch mit den nicht committeten Änderungen des Originals. Das Original läuft unverändert weiter, und der Agent der Abzweigung erfährt, wo der Home-Ordner des Originals liegt, damit er chat-spezifische Worktrees neu anlegt statt sie weiterzuverwenden.
- **In der Cloud ausführen.** Schalte beim Anlegen eines Chats den *Cloud*-Chip neben der Agentenauswahl ein, und der Chat läuft auf [Anthropic Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview) statt in einer lokalen CLI: eine Session in einer Anthropic-Cloud-Sandbox, pro Token über den in Einstellungen ▸ Agenten ▸ Cloud-Chats hinterlegten API-Key abgerechnet, die weiterarbeitet, wenn PopBot beendet wird. Das funktioniert mit jeder Workspace-Wahl: ohne Repo ist die Sandbox leer; vom Repo-Root klont die Sandbox das GitHub-Remote auf dem Branch, auf dem der Root steht (vorher gepusht, falls `origin` ihn noch nicht hat); mit Slot oder Worktree bekommt der Chat wie üblich seinen Branch, PopBot pusht ihn nach `origin`, und die Sandbox klont diesen Branch. Die Sandbox klont mit deinem GitHub-Token (aus den Einstellungen oder dem, mit dem `gh` angemeldet ist), und der Agent bekommt dieses Token in der Sandbox, damit er pushen kann; er soll es aus seinen Antworten heraushalten, und PopBot speichert es nie im Chat. Deine erste Nachricht erzeugt die Session; Antworten, Tool-Aufrufe und Ergebnisse streamen wie bei jedem anderen Agenten in den Chat, und was passiert ist, während PopBot geschlossen war, wird beim nächsten Öffnen des Chats nachgespielt. Tools laufen in der Sandbox ohne Rückfragen (sie ist isoliert; das Einzige, was sie verlässt, ist der Push auf den Branch des Chats). Der Agent committet und pusht auf diesen Branch; *Cloud-Arbeit in den Checkout holen* im Chat-Menü (oder in den Chat-Einstellungen) bringt den lokalen Checkout per Fast-Forward auf diesen Stand. Ein Wechsel von Modell oder Aufwand beendet die Session; die nächste Nachricht startet eine neue, vorbereitet mit dem bisherigen Gespräch. Weder das Schließen des Chats noch das Löschen noch das Beenden von PopBot beendet die Session auf dem Server – so arbeitet sie ohne dich weiter. *Cloud-Chat herunterfahren* im Chat-Menü (☰ oder Rechtsklick auf den Cloud-Chip) beendet sie: Läuft sie noch, wird sie unterbrochen, dann archiviert; nicht Gepushtes geht verloren, und die nächste Nachricht startet eine neue Session, vorbereitet mit dem Gespräch. Nur Claude; Codex Cloud ist nicht angebunden.
- **Auf einem anderen Rechner ausführen.** *Ausführen auf* im Dialog für neue Chats listet die Hosts aus Einstellungen ▸ Hosts: andere Maschinen, auf denen `popbot-host` läuft, ein kleiner Daemon, der die CLIs `claude` und `codex` in eigenen Checkouts ausführt. Wähle einen, und der Dialog zeigt die Repositories und Branches dieses Hosts statt der lokalen: Der Chat läuft im Root des Repositories, in einem Worktree, das der Host auf einem neuen Branch anlegt, oder in einem Scratch-Ordner dort ohne Repository. Auf deinem Rechner wird nichts angelegt; Transkript, Suche, Einstellungen und Berechtigungsregeln bleiben in PopBots Datenbank, und die Antworten, Tool-Aufrufe und Berechtigungsanfragen des Agents strömen wie gewohnt in den Chat (Anhänge werden übertragen). Der Chip des Chats nennt den Host und öffnet seine Einstellungen, in denen das Arbeitsverzeichnis auf dem Host steht. Den Chat zu schließen oder PopBot zu beenden trennt nur die Verbindung: Die Sitzung arbeitet auf dem Host weiter, und der Chat holt beim nächsten Öffnen nach, was er verpasst hat. *Auf dem Host beenden* im Chat-Menü (☰ oder Rechtsklick auf den Chip) beendet sie; die nächste Nachricht startet dort eine neue Sitzung, die dasselbe Gespräch fortsetzt. Ein Wechsel von Modell oder Aufwand tut dasselbe. Tools, die der Host nicht erreicht – der `popbot`-MCP-Server dieses Rechners und die Editor-MCPs der Slots – werden auf einem Host nicht angeboten. Lass ihn auch auf diesem Computer laufen (`http://127.0.0.1:7677`), um einen Agent in einem Checkout arbeiten zu lassen, das PopBot nicht verwaltet. Slots gibt es dort ebenfalls: Ein Host hält pro Repository einen Pool warmer Worktrees (oder ein Wegwerf-Worktree pro Chat), nimmt beim Anlegen eines Chats einen davon, parkt ihn wieder, wenn der Chat auf dem Host beendet wird (ungesicherte Arbeit wird unter dem Namen des Chats gestasht und beim Zurückkommen wieder geholt), und merkt sich über eigene Neustarts hinweg, wer was hält. Der Tab *Hosts* im linken Panel zeigt die Slot-Pools jeder Maschine – dieser Computer zuerst – und die Chats auf jedem Host; ein belegter Slot oder eine Chat-Zeile fokussiert den Chat. Hosts werden in Einstellungen ▸ Hosts hinzugefügt und entfernt, wo auch die Repositories und Slot-Pools jedes Hosts verwaltet werden; dieser Computer steht dort immer und kann nicht entfernt werden.
...
- Das Archiv ist durchsuchbar nach Name, Ticket, Branch und Inhalt, und **Chats durchsuchen** (⌘⇧F / Strg+Umschalt+F oder *Ansicht ▸ Chats durchsuchen…*) durchsucht jedes Transkript, offen wie archiviert: passende Zeilen erscheinen je Chat, beste zuerst, und *Gehe zu* öffnet den Chat – bei Bedarf aus dem Archiv – und scrollt zu dieser Zeile. Filter-Tags grenzen die Suche ein – `ticket:ENG-123`, `cr:67`, `last:week`, `from:user`, `tool:Bash`, `agent:codex`, `in:archive`, `chat:login+bug` (in einem Tag-Wert verbindet `+` Wörter, die alle vorkommen müssen) – getippt oder über die Buttons unter dem Feld gewählt; Tags allein listen die neuesten passenden Einträge. Beim Tippen von `ticket:`, `cr:` oder `chat:` erscheint unter dem Feld eine Liste der Tickets, PRs und Chats, die PopBot kennt; Tab übernimmt einen Eintrag; ein gewählter Chat wird als seine ID eingefügt, sodass Leerzeichen und Satzzeichen im Titel keine Rolle spielen.
...
### PopBot-Tools für Agenten

Der Agent jedes Chats – Claude wie Codex – bekommt einen `popbot`-MCP-Server, damit ein Agent die App so bedienen kann wie du und Chats miteinander reden können:

- **Chats** – `list_chats` (Namen, IDs, Status und welcher Chat gerade aufruft), `create_chat` (ein Slot-Chat auf einem neuen Branch, ein Repo-Root-Chat oder ein Cloud-Chat, wahlweise mit einer ersten Nachricht), `close_chat`, `reopen_chat`.
- **Andere Agenten** – `send_to_chat` schickt eine Nachricht an einen anderen Chat und wartet standardmäßig, bis die Runde dieses Agenten endet, und liefert dessen Antwort zurück; bei einem beschäftigten Chat wird die Nachricht hinter seiner laufenden Arbeit eingereiht. Ein Chat kann sich nicht selbst anschreiben oder schließen. Eine so gesendete Nachricht kommt im anderen Chat mit Absenderangabe an – Name und ID des sendenden Chats – und sagt dem Agenten dort, ob der Absender auf eine Antwort wartet oder wie er ihm zurückschreibt.
- **Arbeit** – `start_code_review` öffnet einen Review-Chat für einen PR (oder ein Swarm-Review) genau wie die Reviews-Liste, Prompt-Vorlage inklusive; `open_ticket_chat` öffnet einen Ticket-Chat auf einem `<du>/<ticket>-<slug>`-Branch, schickt den Start-Ticket-Prompt und setzt das Ticket auf In Progress.
- **Transkripte** – `get_chat_transcript` liest einen Chat (den eigenen oder einen anderen) als nummerierte Einträge, ganz oder als Bereich; `search_chats` findet einen Begriff mit dem Text um jeden Treffer, in einem Chat, über alle offenen oder im ganzen Archiv – ein Trigramm-Volltextindex über alles, was du und der Agent geschrieben habt (mit `from:tool` auch Tool-Aufrufe und ihre Ausgabe), sodass ein Fragment eines Bezeichners oder einer Fehlermeldung genügt. Das Suchfeld des Archivs nutzt denselben Index. `list_refs` liefert die Ticket-Schlüssel, PR-Nummern und Chat-IDs, die eine Suche nennen kann, und `go_to_message` zeigt dir einen Treffer: Es fokussiert den Chat – bei Bedarf aus dem Archiv – und scrollt zur Nachricht.

Der Server lauscht nur auf localhost, auf einem zufälligen Port mit einem pro Start erzeugten Geheimnis in seiner URL, und lässt sich unter *Einstellungen ▸ Agents* abschalten.
...
Der Streifen selbst zeigt jeden offenen Chat auf einmal, egal wie viele es sind. Die Chats rund um den, den du gerade ansiehst, erscheinen als volle Thumbnails; die übrigen schrumpfen zu schmalen Streifen – Statussymbol, der Name von oben nach unten, der Blinker bei laufender Arbeit – dicht gepackt, damit alle im Blick bleiben. Seitliches Scrollen über den Streifen verschiebt den geöffneten Bereich: Auf der einen Seite dehnen sich Streifen zu Thumbnails, auf der anderen schrumpfen Thumbnails zu Streifen. Ein Klick auf einen Streifen öffnet ihn und fokussiert seinen Chat; Ziehen funktioniert bei Streifen wie bei Thumbnails gleich. Über dem oberen Drittel des Streifens wird der Zeiger zur Hand: Dort drücken und ziehen scrollt den Streifen von Hand.
