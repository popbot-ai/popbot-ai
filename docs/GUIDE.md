...
- **Run in the cloud.** When creating a chat, switch on the *Cloud* button beside the agent picker and the chat drives a [Claude Code cloud session](https://claude.ai/code) instead of a local agent: it works with any workspace choice: with no repo the session simply runs remotely; from the repo root the cloud clones the GitHub remote at the branch the root is on (push it first); with a slot or worktree the chat gets its branch as usual, PopBot pushes it to `origin`, and the cloud clones that branch while the slot stays local for teleporting the work back. Your first message starts the session from the chat's terminal (`claude --cloud`), and while that CLI stays attached the conversation is mirrored into the chat column — replies, tool calls and all — and messages you send are typed straight into it. Once the CLI has exited, messages are still queued into the session (`claude -p --cloud`), but replies are only on claude.ai. The session keeps running after PopBot quits. The cloud reaches GitHub through your claude.ai account, not your machine: install the Claude GitHub App from claude.ai/code, or run `/web-setup` once in a terminal `claude` session. The header chip links to the session on claude.ai, and *Teleport into the terminal* in the chat settings pulls its branch and conversation back to your machine. Claude only for now; Codex Cloud is not wired up.
