...
Five session controls matter for long-running work:
...
- **Fork the chat.** From the chat's ☰ menu (or its settings sheet), *Fork chat* starts a second chat at this exact point so the two can diverge: the conversation so far is copied, the agent's own session is forked (Claude and Codex both support this natively, so the fork remembers the work rather than re-reading it), and a chat with a workspace gets its own fresh slot on a `<branch>-fork` branch carrying the original's uncommitted changes. The original continues untouched, and the fork's agent is told where the original's home folder is so it re-creates any chat-specific worktrees instead of reusing them.
- **Run in the cloud.** When creating a chat, switch on the *Cloud* chip beside the agent picker and the chat runs on [Anthropic Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview) instead of a local CLI: a session in an Anthropic cloud sandbox, billed by the token to the API key set in Preferences ▸ Agents ▸ Cloud chats, that keeps working after PopBot quits. It works with any workspace choice: with no repo the sandbox is empty; from the repo root the sandbox clones the GitHub remote at the branch the root is on (pushed first if `origin` does not have it yet); with a slot or worktree the chat gets its branch as usual, PopBot pushes it to `origin`, and the sandbox clones that branch. The sandbox clones with your GitHub token (Preferences, or the one `gh` is signed in with), and the agent is handed that token inside the sandbox so it can push; it is told to keep it out of its replies, and PopBot never stores it in the chat. Your first message creates the session; replies, tool calls and results stream into the chat like any other agent's, and whatever happened while PopBot was closed is replayed when the chat is next opened. Tools run in the sandbox without permission prompts (it is isolated; the only thing that leaves it is the push to the chat's branch). The agent commits and pushes to that branch; *Pull cloud work into the checkout* in the chat menu (or the chat settings) fast-forwards the local checkout to it. Changing the model or effort ends the session; the next message starts a new one primed with the conversation. Closing the chat, deleting it or quitting PopBot never ends the session on the server — that is how it keeps working without you. *Shut down cloud chat* in the chat menu (☰, or a right-click on the Cloud chip) is what ends it: the session is interrupted if it is still working, then archived; anything not pushed is lost, and the next message starts a new session primed with the conversation. Claude only; Codex Cloud is not wired up.
- **Run on another box.** *Run on* in the new-chat dialog lists the hosts from Preferences ▸ Hosts: other machines running `popbot-host`, a small daemon that runs the `claude` and `codex` CLIs in its own checkouts. Pick one and the dialog shows that host's repositories and branches instead of the local ones: the chat runs at the repository's root, in a worktree the host makes on a new branch, or in a scratch folder there with no repository. Nothing is made on your machine; the transcript, search, settings and permission rules stay in PopBot's database, and the agent's replies, tool calls and permission prompts stream into the chat as usual (attachments are sent over). The chat's chip names the host and opens its settings, where the working directory on the host is shown. Closing the chat or quitting PopBot only detaches: the session keeps working on the host, and the chat catches up on what it missed when reopened. *Shut down on host* in the chat menu (☰, or a right-click on the chip) ends it; the next message starts a new session there that resumes the same conversation. Changing the model or effort does the same. Tools the host cannot reach — this machine's `popbot` MCP server and slot editor MCPs — are not offered on a host. Run it on this computer too (`http://127.0.0.1:7677`) to keep an agent working in a checkout PopBot does not manage. Slots work there too: a host keeps a pool of warm worktrees per repository (or a throwaway worktree per chat), takes one for a chat when it is created, parks it again when the chat is shut down on the host (dirty work stashed under the chat's name and popped when it comes back), and remembers who holds what across its own restarts. The *Hosts* tab in the left panel shows every machine's slot pools — this computer first — and the chats on each host; a held slot or a chat row focuses the chat. Hosts are added and removed in Preferences ▸ Hosts, where each host's repositories and slot pools are managed; this computer is always listed there and cannot be removed.
...
- The archive is searchable across name, ticket, branch, and content, and **Search chats** (⌘⇧F / Ctrl+Shift+F, or *View ▸ Search Chats…*) searches every transcript, open and archived: matching lines are shown per chat, best first, and *Go to* opens the chat — reopening it from the archive if needed — and scrolls to that line. Filter tags narrow it — `ticket:ENG-123`, `cr:67`, `last:week`, `from:user`, `tool:Bash`, `agent:codex`, `in:archive`, `chat:login+bug` (in a tag value `+` joins words that must all appear) — typed into the box or picked from the buttons under it; tags alone list the newest matching entries. Typing `ticket:`, `cr:` or `chat:` offers the tickets, PRs and chats PopBot knows in a list under the box; Tab accepts one; a completed chat is inserted as its id, so the title's spacing and punctuation never matter.
...
- **Other agents** — `send_to_chat` posts a message to another chat and, by default, waits for that agent's turn to finish and returns its reply; a busy chat gets the message queued behind its current work. A chat cannot message or close itself. A message sent this way arrives in the other chat attributed to the sending chat — its name and id — and tells that agent whether the sender is waiting for a reply or how to message it back.
...
- **Transcripts** — `get_chat_transcript` reads a chat (its own or another's) as numbered entries, whole or by range; `search_chats` finds a term with the text around each hit, in one chat, every open one, or the whole archive — a trigram full-text index over what you and the agent wrote (tool calls and their output too, with `from:tool`), so a fragment of an identifier or an error message is enough. The archive's own search box uses the same index. `list_refs` gives the ticket keys, PR numbers and chat ids a search can name, and `go_to_message` shows a hit to you: it focuses the chat — reopening it from the archive if needed — and scrolls to the message.
