...
- **Run in the cloud.** When creating a chat, switch on the *Cloud* button beside the agent picker and the chat drives a [Claude Code cloud session](https://claude.ai/code) instead of a local agent: your first message starts the session from the chat's terminal (`claude --cloud`, using the repo's GitHub remote at the branch the repo root is on, so push first), later messages are queued into it as follow-ups, and it keeps running after PopBot quits. The header chip links to the session on claude.ai, and *Teleport into the terminal* in the chat settings pulls its branch and conversation back to your machine. Claude only for now; Codex Cloud is not wired up.
