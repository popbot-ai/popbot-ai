...
const INITIAL_CHATS = [
  {
    id: "c1",
    name: "ENG-20512 · ability cooldown",
    branch: "eng/20512-cooldown-display",
    status: "run",
    timestamp: "active now",
    tokens: { used: 412_000, budget: 1_000_000 },
    snippet: "Patching CooldownView to use server-stamped expiry. Re-running fixture combat-ability-loop…",
    type: "client_test",
    ticket: "ENG-20512",
  },
  {
    id: "c2",
    name: "ENG-20447 · library cache crash",
    branch: "eng/20447-library-cache",
    status: "wait",
    timestamp: "1m ago — needs you",
    tokens: { used: 712_000, budget: 1_000_000 },
    snippet: "Permission needed: agent wants to run `git push origin eng/20447-library-cache` to back up progress.",
    type: "client_test",
    ticket: "ENG-20447",
  },
  {
    id: "c3",
    name: "PR #7401 review",
    branch: "review/7401-ability-resolver",
    status: "run",
    timestamp: "active now",
    tokens: { used: 188_000, budget: 1_000_000 },
    snippet: "Walking through resolver.cs hunk 3/8. Determinism looks good but seed plumbing leaks into UI layer.",
    type: "lite",
    pr: 7401,
  },
  {
    id: "c4",
    name: "Boss arena loadtime",
    branch: "eng/20355-arena-loadtime",
    status: "done",
    timestamp: "12m ago — finished",
    tokens: { used: 540_000, budget: 1_000_000 },
    snippet: "Identified 3 addressable groups loading on bootstrap that shouldn't. Patch ready, opened PR #7402.",
    type: "client_test",
    ticket: "ENG-20355",
  },
  {
    ...
    branch: "eng/20498-inv-flicker",
    status: "idle",
    timestamp: "31m ago — paused",
    tokens: { used: 96_000, budget: 1_000_000 },
    snippet: "Reproduced in fixture inv-stack-resize. Awaiting your direction on stack-count source of truth.",
    type: "client_test",
    ticket: "ENG-20498",
  },
];

const INACTIVE_CHATS = [
...
// Slack mentions / threads to monitor
const SLACK = [
  { ch: "#eng-demo-app",  who: "kira",   t: "2m",  text: "@you any ideas why the cooldown HUD reads 0.0 for one frame? saw your branch", unread: true,  thread: 3, mention: true },
  ...
  { ch: "#eng-demo-app",  who: "tess",   t: "12m", text: "PR #7401 ready for another pass when you're done with the cooldown one", unread: true,  thread: 1, mention: false },
...
// Per-chat live activity feed (most-recent first). For monitor cards.
const CHAT_ACTIVITY = {
  c1: [
...
const StatusGlyph = ({ status }) => {
  const map = { run: "▶", done: "✓", wait: "?", err: "✗", idle: "○" };
  return <span className={`glyph status-${status}`}>{map[status] || "○"}</span>;
};
...
const tokenBarClass = (used, budget) => {
  const p = used / budget;
...
Object.assign(window, { TICKETS, PRS, SLACK, INITIAL_CHATS, INACTIVE_CHATS, CHAT_ACTIVITY, StatusGlyph, fmtTokens, tokenBarClass, avatarColor });
