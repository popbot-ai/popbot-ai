...
export const en = {
  ...
  'search.tag.ticket': 'Ticket chats only; add the key for one ticket (ticket:ENG-123)',
  'search.tag.cr': 'Code-review chats only; add the number for one PR (cr:123)',
  'search.tag.lastWeek': 'Last 7 days',
  'search.tag.lastMonth': 'Last 30 days',
  'search.tag.fromUser': 'What you wrote',
  'search.tag.fromAgent': 'What the agent wrote',
  'search.tag.fromTool': 'Tool calls and their output',
  'search.tag.tool': 'A specific tool (tool:Bash)',
  'search.tag.agent': 'Codex chats (or agent:claude)',
  'search.tag.inArchive': 'Archived chats only',
  'search.tag.chat': 'Chats whose name contains… (chat:login)',
  'work.emptyHint':
  ...
  'auth.signIn.title': 'Sign in to {vendor}',
  'auth.signIn.opening': 'Opening your browser to sign in…',
  'auth.signIn.fallback': 'If the browser didn’t open:',
  'auth.signIn.openLink': 'Open the sign-in page',
  'auth.signIn.pasteCode': 'If the browser shows a code, paste it here:',
  'auth.signIn.continue': 'Continue',
  'auth.signIn.done': 'Signed in.',
  'auth.signIn.failed': 'Sign-in didn’t complete (exit code {code}).',
  'auth.signIn.retry': 'Try again',
  ...
  'chat.ready.signedOut': 'Installed, but not signed in',
  ...
  'prefs.agents.cloud.workspace.desc':
    'Only for a key that is not scoped to a workspace (the check says so): the wrkspc_… id from the Console’s Workspaces page. A key created inside a workspace needs none.',
